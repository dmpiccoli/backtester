...
class Portfolio(Asset):
    ...
    def __init__(self, name: str, begin_date: dt.datetime, start_nav: float = 100000000.0, currency: str = 'BRL',
                 calendar: CalendarType = CalendarType.BR,
                 cash_index: dict[str, str] = {'BRL': 'BZACCETP Index', 'USD': 'LD20TRUU Index'}) -> None:
        ...
        self.market_data = pd.DataFrame(index=[begin_date],
                                        columns=['NAV', 'close', 'r', 'alpha'] + ['cash_' + c for c in
                                                                                  self.cash[begin_date].keys()],
        ...
        if d_1 in self.positions:
            for p in self.positions[d_1]['future'].values():
                tmp = pd.DataFrame(columns=['datetime', 'ticker', 'asset', 'qty', 'price', 'value', 'perc'],
                                   data=[[d_1, p['ticker'], 'future', p['qty'], p['price'],
                                          p['qty'] * p['m'] * p['price'], p['qty'] * p['m'] * p['price'] / nav]])
                if pos.empty:
                    pos = tmp
                else:
        ...
        if date in self.positions:
            for p in self.positions[date]['future'].values():
                tmp = pd.DataFrame(columns=['datetime', 'ticker', 'asset', 'qty', 'price', 'value'],
                                   data=[[date, p['ticker'], 'future', p['qty'], p['price'],
                                          p['qty'] * p['m'] * p['price']]])
                if pos.empty:
                    pos = tmp
                else:
    ...
    def get_positions(self) -> pd.DataFrame:
        ...
        for k1, p1 in self.positions.items():
            for p in self.positions[k1]['future'].values():
                tmp = pd.DataFrame(columns=['datetime', 'ticker', 'asset', 'qty', 'price', 'value'],
                                   data=[[k1, p['ticker'], 'future', p['qty'], p['price'],
                                          p['qty'] * p['m'] * p['price']]])
                if pos.empty:
                    pos = tmp
                else:
            ...
            for p in self.positions[k1]['equity'].values():
                tmp = pd.DataFrame(columns=['datetime', 'ticker', 'asset', 'qty', 'price', 'value'],
                                   data=[[k1, p['ticker'], 'equity', p['qty'], p['price'],
                                          p['qty'] * p['m'] * p['price']]])
                if pos.empty:
                    pos = tmp
                else:
            ...
            if 'future' in self.trades[k1]:
                for t in self.trades[k1]['future']:
                    tmp = pd.DataFrame(columns=['datetime', 'ticker', 'asset', 'qty', 'price'],
                                       data=[[k1, t['ticker'], 'future', t['qty'],
                                              'Settle' if math.isnan(t['price']) else t['price']]])
                    if trades.empty:
                        trades = tmp
                    else:
    ...
    def add_order_equity_perc(self, date: dt.datetime, ticker: str, perc: float):
        ...
        if eq.currency != self.currency:
            c = DataManager().load(ticker=eq.currency + self.currency + ' Curncy')[
                eq.currency + self.currency + ' Curncy']
    ...
    def process(self, end: dt.datetime) -> bool:
        ...
        while process_date <= end:
            ...
            self.cash[process_date] = {
                self.currency: last_nav if process_date == self.begin_date else self.cash[max(self.cash.keys())][
                    self.currency]}
            ...
            try:
                ...
                for p in self.positions[self.last_update]['future'].values():
                    if p['qty'] != 0:
                        ...
                        if fut.currency != fut.settlement_currency:
                        ...
                        # create new position
                        new_pos = {'ticker': p['ticker'], 'currency': p['currency'],
                                   'settlement_currency': p['settlement_currency'], 'qty': p['qty'],
                        ...
                        new_pos['pnl'] = new_pos['pnl'] + (
                                    new_pos['qty'] * (new_pos['price'] - p['price']) * new_pos['m']) * new_pos['c']
                        self.positions[process_date]['future'][new_pos['ticker']] = new_pos

                        # Update provisions if it's different then 0
                        if new_pos['pnl'] != 0.0:
                            new_prov = {'ticker': new_pos['ticker'],
                                        'maturity': fut.calendar.workday(process_date, fut.days2settle),
                ...
                if process_date in self.trades and 'future' in self.trades[process_date]:
                    for t in self.trades[process_date]['future']:
                        ...
                        if fut.settlement_currency != fut.settlement_currency:
                            c = DataManager().load(ticker=fut.currency + fut.settlement_currency + ' Curncy')[
                                fut.currency + fut.settlement_currency + ' Curncy']
                            c = c.get_close(date=process_date)
                        else:
                            c = 1.0
                        ...
                        if t['ticker'] in self.positions[process_date]['future']:
                            ...
                            new_pos = {'ticker': t['ticker'], 'currency': fut.currency,
                                       'settlement_currency': fut.settlement_currency,
                                       'qty': 0, 'price': fut.get_close(date=process_date), 'm': fut.m, 'pnl': 0.0,
                                       'c': c}
                        ...
                        if new_pos['pnl'] != 0.0:
                            if t['ticker'] in self.positions[process_date]['provision']:
                                ...
                                new_prov = {'ticker': new_pos['ticker'],
                                            'maturity': self.calendar.workday(process_date, fut.days2settle),
                ...
                for p in self.positions[self.last_update]['equity'].values():
                    if p['qty'] != 0:
                        ...
                        new_pos = {'ticker': p['ticker'], 'currency': p['currency'],
                                   ...
                                   'qty': p['qty'], 'price': eq.get_close(date=process_date), 'm': p['m'], 'pnl': 0.0,
                                   'c': c}
                ...
                if process_date in self.trades and 'equity' in self.trades[process_date]:
                    for t in self.trades[process_date]['equity']:
                        ...
                        if t['ticker'] in self.positions[process_date]['equity']:
                            ...
                            new_pos = {'ticker': t['ticker'], 'currency': eq.currency,
                                       'settlement_currency': eq.settlement_currency,
                                       'qty': 0, 'price': eq.get_close(date=process_date), 'm': eq.m, 'pnl': 0.0,
                                       'c': c}
                        ...
                        # Pnl and cost updates (if price is nan then trade at settle)
                        cost_bps = abs(t['qty']) * new_pos['m'] * (
                            new_pos['price'] if math.isnan(t['price']) else t['price']) * eq.cost_bps * new_pos['c']
                        cost_unit = abs(t['qty']) * new_pos['m'] * (
                            new_pos['price'] if math.isnan(t['price']) else t['price']) * eq.cost_unit * new_pos['c']
                        pnl = 0.0 if math.isnan(t['price']) else (new_pos['price'] - t['price']) * t['qty'] * new_pos[
                            'm'] * new_pos['c']
                        new_pos['pnl'] = new_pos['pnl'] + (pnl - cost_bps - cost_unit) * new_pos['c']
                        ...
                        if t['ticker'] in self.positions[process_date]['provision']:
                            ...
                            new_prov = {'ticker': new_pos['ticker'],
                                        'maturity': self.calendar.workday(process_date, eq.days2settle),
                                        'currency': eq.settlement_currency, 'value': 0.0, 'c': new_pos['c']}
                        new_prov['value'] = new_prov['value'] - cost_bps - cost_unit \
                                            - t['qty'] * new_pos['m'] * (
                                                new_pos['price'] if math.isnan(t['price']) else t['price']) * c
                        self.positions[process_date]['provision'][new_prov['ticker']] = new_prov
                # endregion Equities
                # region Provision
                ...
                for k in list(self.positions[self.last_update]['provision']):
                    if self.positions[self.last_update]['provision'][k]['maturity'] == process_date:
                        ...
                        if curr_prov != self.currency:
                            c = DataManager().load(ticker=curr_prov + self.currency + ' Curncy')[
                                curr_prov + self.currency + ' Curncy']
                            c = c.get_close(date=process_date)
                        else:
                            c = 1.0
                        self.cash[process_date][self.currency] += self.positions[self.last_update]['provision'][k][
                                                                      'value'] * c
                    else:
                        ...
                        self.positions[process_date]['provision'][
                            self.positions[self.last_update]['provision'][k]['ticker']] = new_prov
                ...
                for k in list(self.positions[process_date]['provision']):
                    if self.positions[process_date]['provision'][k]['maturity'] == process_date:
                        ...
                        if curr_prov != self.currency:
                            ...
                            c = 1.0
                        self.cash[process_date][self.currency] += self.positions[process_date]['provision'][k][
                                                                      'value'] * c
                ...
                for k, i in self.cash[process_date].items():
                    ...
                    self.cash[process_date][k] = self.cash[process_date][k] * (
                                q.get_close(process_date) / q.get_close(self.last_update))
                # endregion Cash
                    ...
                    if k != 'future':
                        for i in p.values():
                            ...
                            if i['currency'] != self.currency:
                                c = DataManager().load(ticker=i['currency'] + self.currency + ' Curncy')[
                                    i['currency'] + self.currency + ' Curncy']
                                c = c.get_close(date=process_date)
                            else:
                                c = 1.0
                ...
                self.market_data = pd.concat([self.market_data,
                                              pd.DataFrame(index=[process_date],
                                                           columns=['NAV', 'close', 'r', 'alpha'] + ['cash_' + c for c
                                                                                                     in self.cash[
                                                                                                         process_date].keys()],
                                                           data=[[total_nav, 1.0, 0.0,
                                                                  total_nav / last_nav - q.get_close(
                                                                      process_date) / q.get_close(
                                                                      max(self.last_update, self.begin_date))] +
